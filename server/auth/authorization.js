...
const isAuthenticated = async (req, res, next) => {
    ...
    if(!bearerToken || typeof bearerToken !== 'string') return res.status(401).json({ errors: [{ msg: 'Invalid token!' }] });
    const session = bearerToken.substring(7);
    const accessFound = await Access.findOne({session,active:true})
    if(!accessFound)  return res.status(401).json({ errors: [{ msg: 'Invalid token!' }] });
    const role = await accessFound.isValidSession(session);
    if(!role) return res.status(401).json({ errors: [{ msg: 'Invalid token!' }] });
...
const isAdminAuthenticated = async (req, res, next) => {
    ...
    const role = await accessFound.isValidSession(session);
    if(role==="admin") next();
    else return res.status(401).json({ errors: [{ msg: 'Invalid token!' }] });
};
