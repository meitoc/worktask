...
taskController.createTask=async(req,res,next)=>{
    ...
    try{
        ...
        await body('description').notEmpty().withMessage('Empty descitption!').run(req);
...
taskController.addReference=async(req,res,next)=>{
    try{
        ...
        for(const e of found){
            ...
            successName.push(e.name);
        }; 
        console.log(successName)
        sendResponse(res,200,true,{users:successName},null,"Add user success")
    }catch(err){
        next(err)
    }
}
...
taskController.deleteReference=async(req,res,next)=>{
    try{
        ...
        console.log(successName)
        sendResponse(res,200,true,{users:successName},null,"Delete user success")
    }catch(err){
        next(err)
    }
}
...
taskController.getAllTasks=async(req,res,next)=>{
    ...
    try{
        const listOfFound= await Task.find(filter).populate("users","-password").sort({ createdAt: sortByTime })
