...
accessController.getOTPChangeEmail=async(req,res,next)=>{
    try{
        ...
        if(!accessFound) return res.status(400).json({ errors: [{ msg: 'Access string is end of service!' }] });
            ...
            try{
                //update User
                const userFound= await User.findOneAndUpdate({_id:accessFound.user,active:true}, {email:accessFound.new_email})
...
//Create a first access for a user
accessController.getOTPAccess=async(req,res,next)=>{
    try{
        //check param by express-validator
        await param('otp_string').notEmpty().withMessage('Invalid access URL!').run(req);
        const errors = validationResult(req);
...
accessController.postForgotPassword=async(req,res,next)=>{
    try{
        ...
        if(!existUser) return res.status(400).json({ errors: [{ msg: 'User is not exist!' }] });
        else try{
            ...
            if(!changedAccess) return res.status(400).json({ errors: [{ msg: 'User is not exist!' }] });
            //send email contains otp
            const sendEmail = await email.sendLoginEmail("Login via email OTP",newAccess.email_otp,targetEmail);
            if(!sendEmail){
