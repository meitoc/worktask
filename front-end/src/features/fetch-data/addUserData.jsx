export default async function addUserData(dataName, dataValue,typeOfData="") {
  ...
  try {
    const response = await fetch(`http://localhost:8000/redirect/9La81A3m223aawsQ/3/account/add-data${typeOfData===""?"":`/${typeOfData}`}`, options);
    console.log(`http://localhost:8000/redirect/9La81A3m223aawsQ/3/account/add-data${typeOfData===""?"":`/${typeOfData}`}`)
