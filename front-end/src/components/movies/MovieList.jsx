...
export default function MovieList(prop) {//prop: movieList, page
  ...
  useEffect(()=>{
    function fetchData () {
      ...
      if(prop.type==="list") url = `http://localhost:8000/redirect/9La81A3m223aawsQ/3/movie/${prop.movieList}?language=en-US&page=${page}`;
      else if(prop.type==="genres") url = `http://localhost:8000/redirect/9La81A3m223aawsQ/3/list/${prop.movieList}`;
      else if(prop.type==="genres_list") url = `http://localhost:8000/redirect/9La81A3m223aawsQ/3/discover/movie?with_genres=${prop.movieList}&language=en-US`;
      else if(prop.type==="collection") url = `http://localhost:8000/redirect/9La81A3m223aawsQ/3/collection/${prop.movieList}?language=en-US`;
      console.log(url);
      fetch(url, options)
