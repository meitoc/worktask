...
export default function FavoriteList(prop) {//prop: movieList, page
    ...
    useEffect(()=>{
        ...
        const fetchData = (i)=>{
            if(i<favoriteLength){
                console.log( `http://localhost:8000/redirect/9La81A3m223aawsQ/3/movie/${favoriteData[i].id}?language=en-US`)
                fetch( `http://localhost:8000/redirect/9La81A3m223aawsQ/3/movie/${favoriteData[i].id}?language=en-US`, options)
