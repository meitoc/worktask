...
export default function Credits(prop) {
  ...
  useEffect(()=>{
    ...
    fetch(`http://localhost:8000/redirect/9La81A3m223aawsQ/3/movie/${prop.movie}/credits?language=en-US`, options)
  ...
  function fetchCreditDetail (creditsId) {
    ...
    fetch(`http://localhost:8000/redirect/9La81A3m223aawsQ/3/credit/${creditsId}`, options)
      .then(response => response.json())
      .then(response => {
